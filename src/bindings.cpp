// mujoco_ros2_sim/src/bindings.cpp
...
/* helper : CtrlInputMap → Python dict --------------------------------*/
static bp::dict map_to_pydict(const CtrlInputMap &m)
{
  bp::dict out;
  for (const auto &kv : m)
    out[kv.first] = kv.second;          // string → double
  return out;
}
...
{
  bp::object mod = bp::scope();          // 현재 bindings 모듈
  auto& registry = ControllerRegistry::instance().map();

  for (auto& kv : registry)
  {
    const std::string pyname = "create_" + kv.first;
...
/* ② 패키지 내부  mujoco_ros_sim.bindings  */
...
BOOST_PYTHON_MODULE(bindings)
{
  ...
  bp::class_<ControllerInterface,
            std::shared_ptr<ControllerInterface>,
            boost::noncopyable                // 복사 금지
            >("ControllerInterface", bp::no_init)
  ...
  bp::converter::registry::push_back(
      &JointDict_from_python::convertible,
      &JointDict_from_python::construct,
      bp::type_id<JointDict>());
